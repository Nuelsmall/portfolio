...
const Talk = ({ hero }: { hero?: boolean }) => {
  ...
  const whatsappLink =
    "https://wa.me/2347035251742?text=Hello%20Emmanuel%2C%20I%20would%20like%20to%20talk.";
  ...
  return (
    ...
    >
      <button
        className={`${
          hero ? "bg-black" : "bg-white"
        } p-2 cursor-pointer items-center rounded-xl justify-center h-12 w-32 text-white flex`}
        onMouseOver={() => setHovered(true)}
        onMouseOut={() => setHovered(false)}
      >
        <p
          className={`${
            hero ? "text-white" : "text-black"
          } relative h-6 w-5 overflow-hidden`}
        >
          <span
        ...
        <p
          className={`${
            hero ? "text-white" : "text-black"
          } relative h-6 overflow-hidden w-20`}
        >
          <span
            className={`flex text-md font-medium *:h-6 *:flex *:items-center *:justify-center absolute w-full flex-col ${
              !hovered && "-translate-y-1/2"
            } top-0 duration-500 transition-transform`}
          >
            <span>Let&apos;s talk</span>
